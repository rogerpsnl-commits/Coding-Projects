  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <Link to="/clients" className="text-sm text-blue-600 hover:underline mb-1 inline-block">
            &larr; Clients
          </Link>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
        <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Client Information</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {contacts.length === 0 ? (
          <div className="py-10 text-center text-slate-400 text-sm">No contacts yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-5 py-3 font-medium text-slate-500">Primary</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {contacts.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-3 text-slate-500">{c.title ?? '—'}</td>
                  <td className="px-5 py-3 text-slate-600">{c.email ?? '—'}</td>
                  <td className="px-5 py-3 text-slate-600">{c.phone ?? '—'}</td>
                  <td className="px-5 py-3">
                    {c.is_primary && (
                      <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">Primary</span>
                    )}
                  </td>
                  <td className="px-5 py-3 text-right">
                    <button
                      onClick={() => openEditContact(c)}
                      className="text-xs text-slate-500 hover:text-blue-600 mr-3"
                    >
                      Edit
                    </button>
                    <button
